...
function shareToWxFriendsJS(){
	var data='{"imgUrl":"'+window.shareData.imgUrl+'","timeLineLink":"'+shareData.timeLineLink+'","tTitle":"'+shareData.tTitle+'","tContent":"'+window.shareData.tContent+'"}';
	android_tw_system.shareToWxFriends(data);
	//android_tw_system.toast("分享成功");
}

function shareToWxTimelineJS(){
	var data='{"imgUrl":"'+window.shareData.imgUrl+'","timeLineLink":"'+window.shareData.timeLineLink+'","tTitle":"'+window.shareData.tTitle+'","tContent":"'+window.shareData.tContent+'"}';
...
function wxAddScore(score,appId,appName){
	if(navigator.userAgent.indexOf("MicroMessenger") >= 0 && document.URL.startWith("wx.1758.com")){		
		jQuery.post("http://wx.1758.com/game/weixin/game/addscore",
				{
					appId:appId,
					score:score,
					appName:appName
				...
				function(data){
					...
					if(data.code == 0){
						if(secondLogin != "cancel"){
							jQuery.post("http://wx.1758.com/game/weixin/user/login",{
								gameUrl:url
							},
							function(data2){
								try{
						...
						if(state.length > 0){
							jQuery.post("http://wx.1758.com/game/weixin/postlogin",
									{
									state:state,
									from:from,
									appId:appId
